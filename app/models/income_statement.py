...
class IncomeStatement(Base):
    ...
    revenue_prior_year = Column(Numeric(20, 2), nullable=True)  # Revenue for same period in prior year
    revenue_prior_year_unit = Column(String, nullable=True)  # Unit for revenue_prior_year
    revenue_growth_yoy = Column(Numeric(10, 4), nullable=True)  # Year-over-year revenue growth percentage
    basic_shares_outstanding = Column(Numeric(20, 2), nullable=True)
    basic_shares_outstanding_unit = Column(String, nullable=True)  # Unit for basic_shares_outstanding (usually "ones")
    diluted_shares_outstanding = Column(Numeric(20, 2), nullable=True)
    diluted_shares_outstanding_unit = Column(String, nullable=True)  # Unit for diluted_shares_outstanding (usually "ones")
    ...
    line_items = relationship("IncomeStatementLineItem", back_populates="income_statement", cascade="all, delete-orphan")
...
class IncomeStatementLineItem(Base):
    ...
    id = Column(String, primary_key=True, index=True)
    income_statement_id = Column(String, ForeignKey("income_statements.id"), nullable=False, index=True)
    ...
    line_order = Column(Integer, nullable=False)  # Order in which line appears in the income statement
