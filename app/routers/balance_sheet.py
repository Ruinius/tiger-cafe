...
def process_balance_sheet_async(
    ...
    from app.utils.financial_statement_progress import (
        update_milestone, initialize_progress, get_progress,
...
async def rerun_financial_statements(
    ...
    return {"message": "Financial statement extraction and classification re-run started", "document_id": document_id}
...
async def get_balance_sheet(
    ...
    if balance_sheet:
        ...
        return {
            "status": "exists",
            "data": balance_sheet_schema.model_dump()
        }
    ...
    if document.analysis_status == ProcessingStatus.PROCESSING:
        ...
        return {
            ...
            "milestones": {
                "step": "extracting",
                "progress": "Processing balance sheet data..."
            }
        }
    ...
    raise HTTPException(
        status_code=404,
        detail="Balance sheet not found for this document"
    )
