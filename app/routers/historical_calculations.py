...
def calculate_and_save_historical_calculations(
    ...
    import json
    calculation_notes_json = json.dumps(results["calculation_notes"]) if results["calculation_notes"] else None
    
    # Check if historical calculation already exists
    existing_calc = db.query(HistoricalCalculation).filter(
...
def get_historical_calculations(
    ...
    calc = db.query(HistoricalCalculation).filter(
        HistoricalCalculation.document_id == document_id
    ).first()
    
    if not calc:
        raise HTTPException(status_code=404, detail="Historical calculations not found for this document")
    
    return calc


@router.post("/{document_id}/historical-calculations/recalculate", response_model=HistoricalCalculationSchema)
def recalculate_historical_calculations(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error calculating historical metrics: {str(e)}")


# Test endpoints (for development)
@router.get("/{document_id}/historical-calculations/test", response_model=HistoricalCalculationSchema)
...
@router.post("/{document_id}/historical-calculations/recalculate/test", response_model=HistoricalCalculationSchema)
