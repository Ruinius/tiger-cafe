...
def calculate_net_working_capital(balance_sheet: BalanceSheet) -> Optional[Decimal]:
    """
    ...
    for item in balance_sheet.line_items:
        ...
        if item.line_category and 'current' in item.line_category.lower() and 'asset' in item.line_category.lower():
            if item.is_operating:
                current_assets_operating += item.line_value
        
        # Check if it's a current liability
        elif item.line_category and 'current' in item.line_category.lower() and 'liability' in item.line_category.lower():
            if item.is_operating:
...
def calculate_net_long_term_operating_assets(balance_sheet: BalanceSheet) -> Optional[Decimal]:
    """
    ...
    for item in balance_sheet.line_items:
        ...
        if 'non-current' in category_lower or ('long' in category_lower and 'term' in category_lower):
            ...
            elif 'liability' in category_lower and item.is_operating and 'total' not in category_lower:
...
def calculate_invested_capital(net_working_capital: Optional[Decimal], 
                                net_long_term: Optional[Decimal]) -> Optional[Decimal]:
    """
...
def calculate_capital_turnover(revenue: Optional[Decimal], invested_capital: Optional[Decimal]) -> Optional[Decimal]:
    """
...
def get_revenue(income_statement: IncomeStatement) -> Optional[Decimal]:
    """
    ...
    for item in income_statement.line_items:
        ...
        if ('revenue' in name_lower or 'revenue' in category_lower or 
            'sales' in name_lower or 'net sales' in name_lower):
...
def get_operating_income(income_statement: IncomeStatement) -> Optional[Decimal]:
    """
...
def get_non_operating_items_between_revenue_and_operating_income(income_statement: IncomeStatement) -> Decimal:
    """
...
def calculate_ebita(income_statement: IncomeStatement, amortization: Optional[Decimal] = None) -> Optional[Decimal]:
    """
    ...
    non_operating_items = get_non_operating_items_between_revenue_and_operating_income(income_statement)
...
def calculate_ebita_margin(ebita: Optional[Decimal], revenue: Optional[Decimal]) -> Optional[Decimal]:
    """
...
def calculate_effective_tax_rate(income_statement: IncomeStatement) -> Optional[Decimal]:
    """
    ...
    for item in income_statement.line_items:
        ...
        if any(term in name_lower for term in [
            ...
            'earnings before income taxes', 'profit before tax',
            'income before income tax expense', 'pre-tax income'
        ]):
            income_before_taxes = item.line_value
        
        # Look for income tax expense
        elif any(term in name_lower for term in [
            'income tax expense', 'income taxes', 'provision for income taxes'
        ]):
            if 'provision' in name_lower:
                provision_for_income_taxes = abs(item.line_value)  # Usually negative in income statement
            else:
...
def calculate_all_historical_metrics(
    ...
    if net_working_capital is None:
        notes.append("Net working capital could not be calculated (missing operating current assets/liabilities)")
    ...
    if effective_tax_rate:
        if effective_tax_rate > Decimal('0.35'):
            notes.append(f"Effective tax rate ({effective_tax_rate * 100:.1f}%) appears high (>35%)")
