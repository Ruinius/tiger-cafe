...
def _throttle():
    ...
    delay = MIN_DELAY_BETWEEN_CALLS + random.uniform(0, MAX_DELAY_BETWEEN_CALLS - MIN_DELAY_BETWEEN_CALLS)
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if is_rate_limit:
                    ...
                    print(f"Rate limit error (attempt {attempt + 1}/{MAX_RETRIES}): {str(e)}. Retrying in {sleep_time:.2f}s...")
                else:
                    jitter = random.uniform(0, delay * 0.1)  # 10% jitter for other errors
                    sleep_time = min(delay + jitter, MAX_RETRY_DELAY)
                    print(f"API call failed (attempt {attempt + 1}/{MAX_RETRIES}): {str(e)}. Retrying in {sleep_time:.2f}s...")
                time.sleep(sleep_time)
...
@_retry_with_backoff
def generate_embedding_safe(text: str, max_chars: int = 20000, task_type: str = "retrieval_document") -> List[float]:
    """
...
@_retry_with_backoff
def generate_content_safe(prompt: str, model_name: Optional[str] = None, temperature: Optional[float] = None) -> str:
    """
...
def get_model_safe(model_name: Optional[str] = None, temperature: Optional[float] = None):
    """
