...
def calculate_dcf(
    ...
    capital_turnovers = [e.get("capital_turnover") for e in recent_entries if e.get("capital_turnover") is not None]
    ...
    for year in range(1, years_to_project + 1):
        ...
        if year <= 5:
            ...
            if mct is None: mct = avg_capital_turnover
            else: mct = Decimal(str(mct))
        else:
    ...
    if mct_terminal is None: mct_terminal = avg_capital_turnover
    else: mct_terminal = Decimal(str(mct_terminal))
